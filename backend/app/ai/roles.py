...
ROLE_SPECS = {
    ...
    for spec in (
        AiRoleSpec(
            ...
            "Исправляет оформление страницы и возвращает предложение для предпросмотра.",
            "text",
        ...
        AiRoleSpec(
            ...
            "Предлагает минимальное полезное число разделов для плоской программы.",
            "text",
            frozenset({"structured_output"}),
            "exact",
            "grouping-v1",
        ...
        AiRoleSpec(
            "exam_chat_reply",
            "Ответ экзаменатора",
            "Ведёт диалог и объясняет материал без выставления оценки.",
            "text",
            frozenset({"streaming"}),
            "none",
            "chat-reply-v1",
        ...
        AiRoleSpec(
            "exam_answer_judge",
            "Проверка ответа",
            "Сопоставляет попытку с эталоном и формирует объяснимый вердикт.",
            "text",
            frozenset({"structured_output"}),
            "exact",
            "answer-judge-v1",
        ...
        AiRoleSpec(
            "exam_chat_memory",
            "Память раздела",
            "Сжимает старые диалоги раздела для связного продолжения.",
            "text",
            frozenset({"structured_output"}),
            "none",
            "chat-memory-v1",
        ...
        AiRoleSpec(
            ...
            "Распознавание речи",
            "Превращает диктовку в редактируемый текст.",
            "speech",
