...
PREPARATION_PROMPT_VERSION = "preparation-estimate-v2"
...
не меняй их. Верни минуты в день от 60 до 480 с шагом 30.

Поле rationale студент увидит прямо под цифрой минут в день, а число дней и элементов
в день на экране уже показаны отдельно — не пересказывай их. Пиши как человек, который
кратко объясняет свой совет, а не как отчёт: 1–2 живых предложения на русском, обычная
грамотная речь без канцелярита и без markdown. Объясни, почему такой темп разумен именно
для этого уровня и цели — например, что мешает делать меньше или зачем закладывать больше.
