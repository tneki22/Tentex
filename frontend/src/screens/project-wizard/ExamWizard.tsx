...
export function ExamWizard({ controller, requestedStep, onStepChange, onActivated }: ExamWizardProps) {
  ...
  async function finishLegacyUpload(
    nextWarnings: string[],
    nextDuplicatesResolution: "kept" | "removed" | null,
    nextDuplicatesResolvedKey: string | null,
  ) {
  ...
  async function continueLegacyTrack(): Promise<
    | { proceed: false }
    | {
        proceed: true;
        warnings: string[];
        duplicatesResolution: "kept" | "removed" | null;
        duplicatesResolvedKey: string | null;
      }
  > {
    ...
    if (form.format !== "unknown") {
      if (form.primaryMode === "text") {
        ...
        const result = await controller.importExam(
          form.rawText,
          form.format as Exclude<ExamFormat, "unknown">,
          alreadyResolved && duplicatesResolution === "removed",
        );
    ...
    return {
      proceed: true,
      warnings: nextWarnings,
      duplicatesResolution: nextDuplicatesResolution,
      duplicatesResolvedKey: nextDuplicatesResolvedKey,
    };
  }
  ...
  async function continueFromUpload() {
    ...
    try {
      ...
      const outcome = await continueLegacyTrack();
      if (!outcome.proceed) return;
      await finishLegacyUpload(outcome.warnings, outcome.duplicatesResolution, outcome.duplicatesResolvedKey);
    } catch (error) {
  ...
  async function resolveDuplicates(resolution: "kept" | "removed") {
    ...
    try {
      ...
      await finishLegacyUpload(nextWarnings, resolution, resolvedKey);
    } catch (error) {
