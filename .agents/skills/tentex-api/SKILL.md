...
Реализованный контракт сверяется по коду и релевантному документу из `docs/architecture/*.md`; индекс — `docs/architecture/README.md`. Модель данных — `backend/app/models.py`, желаемые требования и словарь терминов — `REQUIREMENTS.md`. Для загрузки, разбора, OCR, Typst, версий и публикации материала сначала открой `LIBRARY_FILE_PROCESSING.md`.

Подробности, которые нужны не каждый раз, лежат отдельно: **`references/data-and-worker.md`** — SQLite и сессии, хранилище файлов, единая очередь `BackgroundJob`, worker и исходящие запросы. Открывай, когда правка их задевает.
...
Основные домены включают `projects`, `materials`, `bindings`, `ai`, `exam`, `preparation`, `conspects`, `cards` и `background`; полный подключённый список роутеров смотри в `app/main.py`, а не в этом примере. Роутер не лазит в БД напрямую и не собирает логику — зовёт функцию сервиса. Сервис не знает про `Request`/`Response`.
