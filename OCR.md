...
## Текущая граница на 09.09.2026

Этот файл — тематическое объяснение OCR-веток, не полное руководство по всем входам Библиотеки. Авторитетная матрица форматов, состояний, хранения и запуска вынесена в [LIBRARY_FILE_PROCESSING.md](LIBRARY_FILE_PROCESSING.md); ниже сохранены подробности распознавания и датированная история исправлений.

- Обычный вход Библиотеки — файл, текст, веб-снимок или YouTube-субтитры — регистрирует Материал в `ready_to_process`, но не создаёт задачу разбора: пользователь отдельно нажимает «Подготовить материал». Typst идёт через отдельный вход, автоматически создаёт `typst_compile` и при однозначной точке входа сразу ставит сборку в очередь; OCR он не использует.
- Текущие режимы разбора — рабочие `fast` («Быстро», локальный PP-OCRv5) и `cloud` («Облако», внешняя зрительная модель). В `cloud/auto` цифровой PDF отправляет только найденные вырезы, но страница-скан без текстового слоя уходит наружу целиком; в стратегии `page` целиком уходит каждая страница.
- `Material.status = ready` означает, что конвейер опубликован, а не что все страницы распознаны без замечаний: внутри готового материала могут оставаться `ocr_low`, включая пустую страницу после переживаемой ошибки облачной модели.
- DOCX сейчас читается как одна логическая страница **по абзацам** и картинкам, вложенным в эти абзацы. Таблицы Word, OMML-формулы и прочие структуры вне `document.paragraphs` не извлекаются; изображение с текстом не отправляется в OCR. YouTube-вход сохраняет только доступные субтитры `ru/en` с метками времени — не видео и не распознавание звука.
- При новой ревизии привязки переносятся сопоставлением фрагментов в пределах того же номера страницы. Несопоставленная привязка не исчезает, но получает `orphaned` и требует разбора человеком.
...
После явного запуска обычного разбора оба режима используют общий BackgroundJob,
формат `ParsedPage`, публикацию ревизии и индексирование; расходятся внутри шага
«Распознать страницу». Для PDF с текстовым слоем один классификатор вёрстки
используют `fast` и `cloud/auto`. `cloud/page`, чистый скан и Typst этому правилу
не подчиняются: первые два отправляют растр страницы распознавателю, а Typst
компилируется отдельной задачей без OCR.

```mermaid
flowchart TD
    A[Пользователь загружает обычный файл] --> B["Определяется тип файла\nPDF · DOCX · JPG/PNG · TXT/MD · аудио"]
    B --> C["Материал зарегистрирован\nstatus = ready_to_process; задачи ещё нет"]
    C --> D["Пользователь явно нажимает\n«Подготовить материал» и выбирает режим"]
    D --> E["Создаётся задача разбора\nBackgroundJob(kind=PARSE)"]
    E --> F["Воркер берёт задачу из очереди\napp/materials/worker.py"]
    F --> G["Постранично: iter_pages()\napp/materials/parsers/native.py"]
    G --> H{"Распознать страницу\n(режим «Быстро» или «Облако»)"}
    H --> I["ParsedPage: текст, элементы,\nкоординаты, качество"]
    I --> J["Страница сохраняется в БД\nMaterialPage"]
    J --> K{"Все страницы\nразобраны?"}
    K -- нет --> G
    K -- да --> L["Документ режется на блоки по заголовкам\nMaterialBlock + MaterialFragment"]
    L --> M["Фрагменты попадают в поиск\nFTS5 / BM25"]
    M --> N["Материал готов\nMaterial.status = READY; возможны страницы ocr_low"]
```
...
| `.docx` | `python-docx`, одна логическая страница; текущий путь читает абзацы и их картинки без OCR, но не таблицы/OMML |
...
`Material.status` становится `READY`: это успешная публикация конвейера, а не
гарантия безошибочного текста. Внутри могут остаться страницы `ocr_low`, в том
числе пустая страница после переживаемой ошибки облачной модели. Если материал
помечен как файл с ответами, тут же запускается автопривязка ответов к вопросам
...
| DOCX | **Нет** — это не PDF; текущий `python-docx`-путь читает `document.paragraphs` и связанные картинки, без таблиц/OMML и без OCR |
...
пытается перенести `transfer_bindings_on_revision`: фрагменты сопоставляются
только в пределах того же номера страницы, а ненайденная связь получает
`orphaned` и требует ручной проверки. Новые формулы и схемы добавляются в
новую ревизию; история старой ревизии не переписывается.
...
- **Не поддержаны и не имитируются**: подключение через собственный протокол
  Ollama без OpenAI-совместимого endpoint, автоматическое переключение на
  второй провайдер и любая модальность помимо текста/речи/зрения. Отказ не
  уезжает к другому поставщику: одиночная переживаемая осечка целой страницы
  сохраняется пустой `ocr_low` (осечка пачки вырезов оставляет исходные вырезы),
  а непереживаемая или третья подряд ошибка останавливает задачу;
...
DOCX — ZIP со структурированным XML, но текущий парсер читает не весь пакет:
он проходит только `document.paragraphs` и картинки, связанные с этими
абзацами. Поэтому выбор `fast/cloud` здесь игнорируется и OCR не вызывается;
`pdf_layout.py` и правка про PDF-боксы `picture`/`formula` к DOCX тоже не
применяются. Это нативное чтение доступной части Word, а не обещание полного
восстановления любого DOCX.

```
1. iter_pages видит расширение .docx → сразу _docx_page(), режим (fast/cloud)
   в этой ветке не смотрится вовсе.
2. `python-docx` читает `document.paragraphs`; таблицы, OMML-математика, колонтитулы, текстовые поля и другие структуры вне этой коллекции не обходятся.
...
**Итог:** результат `ParsedPage` использует тот же общий формат, но содержит
только извлечённые абзацы, восстановленные списки и найденные в них картинки.
Word-таблицы и OMML-формулы сейчас пропускаются; DOCX, состоящий из одних
скан-картинок, не превращается в распознанный текст. Картинка остаётся
`[Изображение]`, потому что ни «Быстро», ни «Облако» в этой ветке не
вызываются. Для полного разбора такой материал нужно пересохранить как PDF
либо вынести изображение в отдельный поддерживаемый вход.
...
Потому что текущая ветка DOCX не вызывает распознаватель: она читает только
`document.paragraphs` и связанные с ними картинки. Выбор между «Быстро» и
«Облако» результата не меняет; image-only DOCX от этого автоматически не
распознаётся.
...
(Материалы → «Обработка» → «Запустить заново»). Перенос привязок при повторном
разборе выполняется по совпадению фрагментов на том же номере страницы. То,
что не удалось сопоставить, не считается перенесённым: связь становится
`orphaned` и должна быть проверена вручную; старая ревизия остаётся в истории.
...
Отдельного адаптера протокола Ollama нет. Подключить можно только endpoint,
который выполняет используемый Tentex контракт OpenAI Chat Completions с
графическими частями и structured output. Автоматического переключения на
резервного провайдера тоже нет. Одиночная переживаемая осечка может оставить
`ocr_low` и не остановить весь документ; непереживаемая или третья подряд
ошибка завершает задачу с ошибкой.
