...
- [ ] `check_exam_chat.py` по образцу `check_ai_gateway.py`: временная SQLite и
...
- [ ] Записать фактический контракт в `docs/architecture/exam-chat.md`: таблицы,
      HTTP, формат кадров SSE, лесенка и её пороги, поведение при выключенных
      моделях, коды ошибок. Только по реально сделанному.
- [ ] Правку `PLAN.md` (статус Среза C) и `AGENTS.md` («Где сейчас находимся»)
      подготовить текстом и **спросить** разрешение, а не вносить молча.
- [ ] Коммит: `feat: exam answer checking ladder and attempt history`.
