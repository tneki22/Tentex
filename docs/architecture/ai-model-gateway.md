...
повторы клиента выключены, потому что их ведёт сам шлюз и учитывает в `AiRun`.

`ModelGateway.complete` даёт вызову четыре попытки с паузами `2 / 6 / 15` секунд
(`RETRY_BACKOFF_SECONDS`; длина последовательности и задаёт число повторов, тесты
передают нулевые паузы). Повторяются только временные коды — `ai_provider_unavailable`,
`ai_empty_response`, `ai_rate_limited`, `ai_timeout`: дешёвые зрительные модели живут в
общем пуле провайдера, и 429 там прилетает из-за чужой нагрузки, а не из-за нашего
расхода. Разбор материала — это сотни вызовов подряд, и без повтора один чужой всплеск
ронял всю обработку. `ai_invalid_credentials` и лимиты стоимости не повторяются никогда:
ключ и кошелёк от ожидания не чинятся. Ответ, не прошедший структурную проверку, тоже
переспрашивается — модели показывают её ошибку и просят прислать исправленный JSON.

Каталог является бесплатной проверкой соединения. Вызов структурного consumer передаёт
strict JSON Schema и затем независимо проверяет результат Pydantic-моделью Tentex.
...
```text
POST /api/projects/{project}/program/ai-import-repair/preflight
POST /api/projects/{project}/program/ai-import-repair
POST /api/projects/{project}/program/ai-import-repair/apply
```

Пересобрано 22.08.2026: раньше функция могла только переименовывать пункты один в один и
не умела чинить билеты или проставлять подпункты. Пересмотрено `backend/app/projects/import_repair.py`.

Снимок — не плоский список вопросов, а пронумерованные позиции дерева: для формата
билетов это сами билеты (`SECTION`+`TICKET`) и их вопросы/задачи; для плоского формата —
только вопросы/задачи. Текст каждой позиции включает уже расставленные подпункты
(`SUBPOINT`-детей) построчными буллетами — так повторный запуск видит прежний результат.

Ответ модели — дерево, а не список строк: `items` — вопросы/задачи (`kind`, `title`,
`subpoints`, `source_indices`) либо билеты с вложенными `items`; `dropped` — позиции,
признанные не вопросом (заголовок, шаблон билета), с `reason`. `source_indices` —
номера позиций входного списка, из которых получен пункт: один номер у одного пункта —
переименование; один номер у нескольких пунктов — разбиение; несколько номеров у одного
пункта — слияние. Каждая входная позиция обязана встретиться ровно в одном месте —
в каком-то `source_indices` или в `dropped`, не в обоих и не в нуле — это и есть замена
старой проверке «число пунктов не изменилось»: количество пунктов теперь свободно
меняется, а под контролем — полнота разбора источника.

Apply сопоставляет каждый пункт ответа со старым узлом по чистому одиночному
`source_indices` (не по тексту — формулировка как раз могла измениться) и переиспользует
его id, сохраняя привязки/эталон/историю. Пункт из разбиения или слияния получает новый
узел; узел, чья позиция не попала ни в один пункт ответа, архивируется, а не удаляется
(тот же `is_in_current_program=False, is_archived=True`, что и у `active_exam_import`).
Подпункты вопроса реконструируются по позиции внутри него: при том же или большем числе
пункты переиспользуют id по порядку, лишние старые архивируются. Действие
