...
`list_conspect_summary` обходит дерево программы в DFS-порядке (сортировка соседей по `(sort_order, str(id))`, как в `program.py`), берёт только `topic`/`subpoint` из `is_in_current_program=true`, `is_archived=false`, с непустым сохранённым текстом. `position` — 1-based номер среди возвращённых записей, не индекс в дереве.
