...
- `WorkspaceState.layout` — проверенная API раскладка: выбранный узел, раскрытые узлы, ширина
  дерева, от одной до трёх рабочих зон с вкладками и их веса. Зона может быть пустой: это
  штатное состояние первого входа или результат закрытия последней вкладки.
