...
- PDF в режиме «Учебник»: CPU-worker постранично отправляет рендер в локальный HTTP
  ...
  precision `fp16`. Связка PP-StructureV3 + PP-FormulaNet_plus-M с моделью разметки
  PP-DocLayout_plus-L возвращает заголовки, текст, LaTeX-формулы, Markdown-таблицы и
  изображения в общую модель. Модель разметки обязана выделять формулы отдельным
  классом (`PP-DocLayout_plus-L`, а не блочная `PP-DocBlockLayout`), иначе
  `use_formula_recognition` не получает формульных областей и формула уезжает в
  обычное распознавание текста. Фактический исполнитель сообщается через health.
