...
Фактический контракт на 10 августа 2026 года. Экзаменационный путь и общая часть 5B
реализованы. Единственный незакрытый hardware-gated пункт — parser «Учебник» на
PaddleOCR-VL: он остаётся видимым и недоступным, пока GPU runtime не пройдёт отдельную
приёмку. Облачные режимы, индекс и привязки не входят в этап 5.
...
- `MaterialPage`, `MaterialFragment`, `MaterialBlock` — версионированный результат
  разбора; bbox нормализован в `0..1`.
...
- PDF: PyMuPDF, outline и нативные координаты; страницы с менее чем 40 непробельными
  символами рендерятся в 2× и идут в PP-OCRv5.
...
- «Учебник» / PaddleOCR-VL остаётся disabled: RTX 5060 Laptop есть, но GPU не проброшена
  в worker, а совместимый production runtime не принят.
