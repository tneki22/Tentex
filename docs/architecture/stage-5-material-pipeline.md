...
- Режим «Учебник» (PP-StructureV3 + PP-FormulaNet_plus-M на отдельном GPU-сервисе,
  распознавание формул в LaTeX и разметка таблиц) снят при стабилизации экзамена —
  описание связки, что делал каждый компонент и как вернуть контур, в
  `docs/archive/gpu-ocr-textbook.md`. Формулы сейчас не распознаются ничем — принятый
  регресс; уже готовый LaTeX по-прежнему рендерится KaTeX.
