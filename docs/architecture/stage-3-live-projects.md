...
через обычный `/api`. На границе самого этапа 3 экзаменационная ветка импортировала вставленный
список и активировала проект, а учебниковая сохраняла возобновляемый черновик, паспорт и ручную
программу без загрузки файла. С этапа 5 это историческое ограничение снято: учебниковый мастер
работает с реальными источниками и активирует проект, в том числе с пустой программой.
...
| `POST` | `/api/wizard-drafts/{project_id}/activate` | проверить паспорт и активировать проект; с этапа 5 экзаменационный и учебниковый черновики могут иметь пустую Программу |
...
| `POST` | `/api/projects/{project_id}/program-nodes/{node_id}/swap` | поменять два узла местами одной отменяемой командой |
...
| 5, материалы | общий `activate` нейтрален к источникам и допускает пустую Программу; обязательные входы проверяет конкретная ветка мастера |
...
### Исправление вопросов в иерархии (04.09.2026)

ИИ получает полное активное дерево в `program_context`: стабильные id, parent_id,
типы, путь названий предков, порядок, целевой уровень, подпункты и только наличие /
длину эталонного ответа. Обычные разделы и подразделы остаются неизменяемыми.
Например, «Математика → Интегралы → Вопрос 1» после исправления текста сохраняет
все три id и того же родителя вопроса. Ответы, привязки и попытки остаются у того же id.

Wire-список допускает одновременно билеты и самостоятельные вопросы / задачи:
у последних `ticket_index=null`. Исправление не переносит вопросы между билетами
и не объединяет разные ветви. Разделение создаёт новые узлы у исходного родителя;
удаление архивирует исходные узлы. Применение сохраняет полный снимок для undo.
Хеш снимка включает структуру и id, поэтому смена родителя также делает результат
устаревшим. Фоновые job/preflight/result/error пути сохранены. Логи отмечают начало
вызова модели и переход к проверке с длительностью без текстов программы.


Предпросмотр возвращает `source_context` — тот же полный контекст, который
получает модель. Disclosure отображает его целиком с путями и id, включая обычные
разделы. У сохранённого билета нельзя удалить исходный вопрос ни ответом модели,
ни ручной правкой apply. Удалённый билет восстанавливается вместе с исходными
вопросами; отдельный вопрос удалённого билета требует сначала вернуть билет.
